...
(define (apply-generic op . args)
  (define (higher x y)
    (let ((raise-x (get 'raise (list (type-tag x))))
          (raise-y (get 'raise (list (type-tag y)))))
       (cond ((and raise-x (not raise-y)) #f)
            ((and (not raise-x) raise-y) #t)
            (else
             (higher (raise x) (raise y))))))
...
(define (install-rational-package)
  ...
  (put 'raise '(rational)
       (λ (x) (make-complex-from-real-imag (/ (numer x) (denom x)) 0)))
  (put 'make 'rational
